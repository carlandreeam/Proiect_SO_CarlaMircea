    ...
    while ((entry = readdir(dir_in)) != NULL)
    {
        ...
        else if (S_ISREG(st_file.st_mode)) // pt alte fisiere regulate
        {
            ...
            else if (pid == 0)
            {
                ...
                int nr_linii = numarare_linii(fd_out, file_out);
                char continut[st_file.st_size];
                close(pipe_stdin[0]);
                close(pipe_stdout[1]);
                close(pipe_stdout[0]);

                if (read(fd, continut, sizeof(continut)) != sizeof(continut))
                {
                    perror("error read");
                    exit(-1);
                }
                if (write(pipe_stdin[1], continut, sizeof(continut)) != sizeof(continut))
                {
                    perror("error write");
                    exit(-1);
                }
            ...
            else
            {
                ...
                char buff[1024];
                if (read(pipe_stdout[0], buff, sizeof(buff)) == -1)
                {
                    perror("error read");
                    exit(-1);
                }
